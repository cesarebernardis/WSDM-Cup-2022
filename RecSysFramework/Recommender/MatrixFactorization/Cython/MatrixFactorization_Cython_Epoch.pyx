...
cdef class MatrixFactorization_Cython_Epoch:
    ...
    cdef float init_mean, init_std_dev, MSE_negative_interactions_quota, MSE_sample_negative_interactions_flag

    cdef int batch_size, max_trials

    cdef int algorithm_is_funk_svd, algorithm_is_asy_svd, algorithm_is_BPR, algorithm_is_WARP

    cdef int[:] URM_train_indices, URM_train_indptr, profile_length
    cdef float[:] URM_train_data

    cdef float[:,:] USER_factors, ITEM_factors
    cdef float[:] USER_bias, ITEM_bias, GLOBAL_bias
    cdef int[:] factors_dropout_mask
    cdef int dropout_flag
    cdef float dropout_quota

    # Mini-batch sample data
    cdef float[:,:] USER_factors_minibatch_accumulator, ITEM_factors_minibatch_accumulator
    cdef float[:] USER_bias_minibatch_accumulator, ITEM_bias_minibatch_accumulator, GLOBAL_bias_minibatch_accumulator
    ...
    cdef float [:,:] sgd_cache_I, sgd_cache_U, sgd_cache_bias_I, sgd_cache_bias_U, sgd_cache_bias_GLOBAL
    cdef float gamma

    cdef float [:,:] sgd_cache_I_momentum_1, sgd_cache_I_momentum_2
    cdef float [:,:] sgd_cache_U_momentum_1, sgd_cache_U_momentum_2
    cdef float [:,:] sgd_cache_bias_I_momentum_1, sgd_cache_bias_I_momentum_2
    cdef float [:,:] sgd_cache_bias_U_momentum_1, sgd_cache_bias_U_momentum_2
    cdef float [:,:] sgd_cache_bias_GLOBAL_momentum_1, sgd_cache_bias_GLOBAL_momentum_2
    ...
    ALGORITHM_NAME_VALUES = ["FUNK_SVD", "ASY_SVD", "MF_BPR", "MF_WARP"]


    def __init__(self, URM_train, n_factors=1, algorithm_name=None,
                 batch_size=1, negative_interactions_quota=0.5, dropout_quota=None,
                 learning_rate=1e-3, use_bias=False, max_trials=10,
                 user_reg=0.0, item_reg=0.0, bias_reg=0.0, positive_reg=0.0, negative_reg=0.0,
                 verbose=False, random_seed=None, init_mean=0.0, init_std_dev=0.1,
        ...
        self.URM_train_data = np.array(URM_train.data, dtype=np.float32)
    ...
    def _init_latent_factors(self):
        ...
        self.USER_factors = np.random.normal(self.init_mean, self.init_std_dev, (n_user_factors, self.n_factors)).astype(np.float32)
        self.ITEM_factors = np.random.normal(self.init_mean, self.init_std_dev, (n_item_factors, self.n_factors)).astype(np.float32)

        self.USER_factors_minibatch_accumulator = np.zeros((n_user_factors, self.n_factors), dtype=np.float32)
        self.ITEM_factors_minibatch_accumulator = np.zeros((n_item_factors, self.n_factors), dtype=np.float32)

        if self.use_bias:
            self.USER_bias = np.zeros(self.n_users, dtype=np.float32)
            self.ITEM_bias = np.zeros(self.n_items, dtype=np.float32)
            self.GLOBAL_bias = np.zeros(1, dtype=np.float32)

            self.USER_bias_minibatch_accumulator = np.zeros(self.n_users, dtype=np.float32)
            self.ITEM_bias_minibatch_accumulator = np.zeros(self.n_items, dtype=np.float32)
            self.GLOBAL_bias_minibatch_accumulator = np.zeros(1, dtype=np.float32)
    ...
    def _init_adaptive_gradient_cache(self, sgd_mode, gamma, beta_1, beta_2):
        ...
        if sgd_mode=='sgd':
            ...
            # Adagrad and RMSProp
            self.sgd_cache_I = np.zeros((self.ITEM_factors.shape[0], self.n_factors), dtype=np.float32)
            self.sgd_cache_U = np.zeros((self.USER_factors.shape[0], self.n_factors), dtype=np.float32)

            self.sgd_cache_bias_I = np.zeros((self.n_items, 1), dtype=np.float32)
            self.sgd_cache_bias_U = np.zeros((self.n_users, 1), dtype=np.float32)
            self.sgd_cache_bias_GLOBAL = np.zeros((1, 1), dtype=np.float32)

            # Adam
            self.sgd_cache_I_momentum_1 = np.zeros((self.ITEM_factors.shape[0], self.n_factors), dtype=np.float32)
            self.sgd_cache_I_momentum_2 = np.zeros((self.ITEM_factors.shape[0], self.n_factors), dtype=np.float32)

            self.sgd_cache_U_momentum_1 = np.zeros((self.USER_factors.shape[0], self.n_factors), dtype=np.float32)
            self.sgd_cache_U_momentum_2 = np.zeros((self.USER_factors.shape[0], self.n_factors), dtype=np.float32)

            self.sgd_cache_bias_I_momentum_1 = np.zeros((self.n_items, 1), dtype=np.float32)
            self.sgd_cache_bias_I_momentum_2 = np.zeros((self.n_items, 1), dtype=np.float32)

            self.sgd_cache_bias_U_momentum_1 = np.zeros((self.n_users, 1), dtype=np.float32)
            self.sgd_cache_bias_U_momentum_2 = np.zeros((self.n_users, 1), dtype=np.float32)

            self.sgd_cache_bias_GLOBAL_momentum_1 = np.zeros((1, 1), dtype=np.float32)
            self.sgd_cache_bias_GLOBAL_momentum_2 = np.zeros((1, 1), dtype=np.float32)
    ...
    def epochIteration_Cython(self):
        ...
        elif self.algorithm_is_WARP:
            self.epochIteration_Cython_WARP_SGD()
    ...
    def epochIteration_Cython_FUNK_SVD_SGD(self):
        ...
        cdef long factor_index, num_current_batch, num_sample_in_batch
        cdef float prediction, prediction_error
        cdef float local_gradient_item, local_gradient_user, local_gradient_bias_item, local_gradient_bias_user, local_gradient_bias_global

        cdef float H_i, W_u, cumulative_loss = 0.0
        ...
        iterations = range(num_total_batch)
        if self.verbose:
            iterations = tqdm.tqdm(iterations)

        for num_current_batch in iterations:
            ...
            if self.verbose:
                iterations.set_postfix_str("MSE loss: {:.2E}".format(
    ...
    def epochIteration_Cython_ASY_SVD_SGD(self):
        ...
        cdef MSE_sample sample
        cdef long num_current_batch, num_sample_in_batch
        cdef float prediction, prediction_error
        cdef float local_gradient_item, local_gradient_user, local_gradient_bias_item, local_gradient_bias_user, local_gradient_bias_global

        cdef float[:] user_factors_accumulated = np.zeros(self.n_factors, dtype=np.float32)
        cdef long start_pos_seen_items, end_pos_seen_items, item_id, factor_index, item_index, user_index

        cdef float H_i, W_u, cumulative_loss = 0.0, denominator
        ...
        for num_current_batch in iterations:
            ...
            if self.verbose:
                iterations.set_postfix_str("MSE loss: {:.4f}".format(
                    cumulative_loss / (num_current_batch * self.batch_size + 1)))
    ...
    def epochIteration_Cython_BPR_SGD(self):
        ...
        cdef long u, i, j
        cdef long factor_index, num_current_batch, num_sample_in_batch
        cdef float x_uij, sigmoid_user, sigmoid_item, local_gradient_i, local_gradient_j, local_gradient_u

        cdef float H_i, H_j, W_u, cumulative_loss = 0.0
        ...
        for num_current_batch in iterations:
            ...
            if self.verbose:
                iterations.set_postfix_str("BPR loss: {:.4f}".format(
                    cumulative_loss / (num_current_batch * self.batch_size + 1)))


    def epochIteration_Cython_WARP_SGD(self):

        # Get number of available interactions
        cdef long num_total_batch = int(self.n_users / self.batch_size) + 1

        cdef WARP_sample sample
        cdef long u, i, j
        cdef long factor_index, num_current_batch, num_sample_in_batch
        cdef float x_uij, sigmoid_user, sigmoid_item, local_gradient_i, local_gradient_j, local_gradient_u
        ...
        for num_current_batch in iterations:
            ...
            for num_sample_in_batch in range(self.batch_size):
                ...
                # Uniform user sampling with replacement
                sample = self.sampleWARP_Cython()

                self._add_BPR_sample_in_minibatch(BPR_sample(sample.user, sample.pos_item, sample.neg_item))
                ...
                #for factor_index in range(self.n_factors):
                #    if self.factors_dropout_mask[factor_index]:
                #        x_uij += self.USER_factors[u, factor_index] * (
                ...
                for factor_index in range(self.n_factors):
                    if self.factors_dropout_mask[factor_index]:
                        ...
                        # Compute gradients
                        local_gradient_u = sample.weight * (H_j - H_i) + self.user_reg * W_u
                        local_gradient_i = sample.weight * (-W_u) + self.positive_reg * H_i
                        local_gradient_j = sample.weight * (W_u) + self.negative_reg * H_j

                        self.USER_factors_minibatch_accumulator[u, factor_index] -= local_gradient_u
                        self.ITEM_factors_minibatch_accumulator[i, factor_index] -= local_gradient_i
                        self.ITEM_factors_minibatch_accumulator[j, factor_index] -= local_gradient_j

                cumulative_loss += sample.weight * x_uij
            ...
            if self.verbose:
                iterations.set_postfix_str("WARP loss: {:.4f}".format(
                    cumulative_loss / (num_current_batch * self.batch_size + 1)))
    ...
    cdef float adaptive_gradient(self, float gradient, long user_or_item_id, long factor_id, float[:,:] sgd_cache, float[:,:] sgd_cache_momentum_1, float[:,:] sgd_cache_momentum_2):
    ...
    cdef WARP_sample sampleWARP_Cython(self):

        cdef WARP_sample sample = WARP_sample(-1, -1, -1, 0.)
        cdef long index, start_pos_seen_items, end_pos_seen_items
        ...
        index = rand() % n_seen_items
        sample.pos_item = self.URM_train_indices[start_pos_seen_items + index]
        fi = np.matmul(self.USER_factors[sample.user, :], self.ITEM_factors[sample.pos_item, :])

        fj = fi - 10.
        while fj <= fi - 1 and n_trials <= self.max_trials:
            ...
            fj = np.matmul(self.USER_factors[sample.user, :], self.ITEM_factors[sample.neg_item, :])
            n_trials += 1

        if n_trials <= self.max_trials:
            sample.weight = np.cbrt((self.n_items - n_seen_items) / n_trials) / np.cbrt(self.n_items - n_seen_items)
